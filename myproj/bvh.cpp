...
using namespace std;
/* c'est peut-�tre mieux d'utiliser des structs pour les aabb,
*  mais je ne sais pas comment g�rer les structs entre .h / .c
...
// �a sert � rien, � enlever, c'est rien que pour l'exemple de la sphere qui entre en contact avec le tissu
...
// check overlappings
 ...
 // draw box
void Aabb::draw() {
	// attention cette fonction <doit> etre entour�e de glbegin et glend,
	// je ne le mets pas � l'interieur de cette fonction car trop couteux --> plutot le mettre au d�but et fin d'une fonction qui
...
void Node::update() {
	...
	// sinon, aabb est form� par les plus petites et plus grandes coordonn�es des enfants. (box.unir) 
	if (isLeaf) {
		box.updateBox(); 
		return;
	}
	...
	for (int i = 0; i < 3; i++)
	{
			...
			if (pt1[0] != myp && pt1[1] != myp && pt1[2] != myp) { // le point pt2[j] ne fait pas partie du triangle 1
				...
				if ( length(bary) != 0) { // length == 0 <=> null
					...
					// j'ajoute la r�puslion ( => moins de collisions )
					if (I > 0.1 *d / DT) {
